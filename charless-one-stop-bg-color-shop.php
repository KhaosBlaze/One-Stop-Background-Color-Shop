...
function osbcs_options_page() {
  add_options_page('One Stop Background Color Shop!', 'One Stop Background Color Shop', 'manage_options', 'one-stop-bg-color-shop', 'osbcs_option_page_html');
}
...
function osbcs_custom_css(){
  $color = get_option('osbcs_color'); //Fetch color
...
{
  if ( ! current_user_can( 'manage_options' ) ) {return; }//Kick the user back if they don't have the correct permissions
?>
